...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bGift)
{
	...
	if(bConquest)
	{
		if(GetID() == GC.getGame().getActivePlayer())
		{
			...
			GC.messageCity(0, pOldCity->GetIDInfo(), GetID(), true, GC.getEVENT_MESSAGE_TIME(), strBuffer/*, "AS2D_CITYCAPTURE", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pOldCity->getX(), pOldCity->getY(), true, true*/);
		}
				...
				if(GET_PLAYER((PlayerTypes)iI).isAlive())
				{
...
#endif
					{
						if(pOldCity->isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
						{
							...
							GC.messageCity(0, pOldCity->GetIDInfo(), ((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), strBuffer/*, "AS2D_CITYCAPTURED", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pOldCity->getX(), pOldCity->getY(), true, true*/);
						}
					}
				}
			}
		...
		if(lResult == 0)
		{
			...
			else if(iCaptureGold > 0 || iCaptureCulture > 0 || iCaptureGreatWorks > 0)
			{
				if (iCaptureCulture == 0 && iCaptureGreatWorks == 0)
				{
					...
					GC.messageCity(0, pNewCity->GetIDInfo(), GetID(), true, GC.getEVENT_MESSAGE_TIME(), strBuffer);
				}
				else
				{
...
void CvPlayer::disbandUnit(bool)
{
	...
	if(pBestUnit != NULL)
	{
		if(GetID() == GC.getGame().getActivePlayer())
		{
			...
			GC.messageUnit(0, pBestUnit->GetIDInfo(), GetID(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer);//, "AS2D_UNITDISBANDED", MESSAGE_TYPE_MINOR_EVENT, pBestUnit->getUnitInfo().GetButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pBestUnit->getX(), pBestUnit->getY(), true, true);
		}
...
void CvPlayer::GetDiplomaticInfluencePerTurn(int* influenceThisTurn, int* iNumMinorCapitalsControlled) const
{
	...
	// check competitions
	{
		...
		{
			influenceT100 += 100 * GC.getGame().getCompetition((MiniCompetitionTypes)i).GetReward(YIELD_DIPLOMATIC_SUPPORT, GetID());
		}
	}
...
void CvPlayer::GetScientificInfluencePerTurn(int* influenceThisTurn) const
{
	...
	// check our owned cities
	{
		...
		for (const CvCity* pLoopCity = firstCity(&iCityLoop); pLoopCity != NULL; pLoopCity = nextCity(&iCityLoop))
		{
			influenceT100 += 100 * pLoopCity->getScientificInfluence();
		}
	}
	...
	// check competitions
	{
		for (int i = 0; i < NUM_COMPETITIONS; ++i)
		{
			influenceT100 += 100 * GC.getGame().getCompetition((MiniCompetitionTypes)i).GetReward(YIELD_SCIENTIFIC_INSIGHT, GetID());
		}
	}
...
void CvPlayer::raze(CvCity* pCity)
{
	...
	if(GetID() == GC.getGame().getActivePlayer())
	{
		...
		GC.messageCity(0, pCity->GetIDInfo(), GetID(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer/*, "AS2D_CITYRAZE", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pCity->getX(), pCity->getY(), true, true*/);
		...
		if(GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if(iI != GetID() && iI == GC.getGame().getActivePlayer())
			{
				if(pCity->isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
				{
					...
					GC.messageCity(0, pCity->GetIDInfo(), ((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer/*, "AS2D_CITYRAZED", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pCity->getX(), pCity->getY(), true, true*/);
				}
			}
		}
	}
...
GoodyTypes getGoodyHut(int numPopped, int numGoodyHuts)
{
	return (GoodyTypes)(numPopped % numGoodyHuts);
}
...
void CvPlayer::found(int iX, int iY)
{
	...
	CardsAdd(CARD_ANCIENT_UNITS_OBSIDIAN_ARROWS);
	CardsAdd(CARD_ANCIENT_UNITS_WAY_OF_THE_WARRIOR);
	CardsAdd(CARD_ANCIENT_UNITS_SCOUTING);
	CardsAdd(CARD_ANCIENT_UNITS_SLAVES);
	CardsAdd(CARD_ANCIENT_UNITS_GREEK_FIRE);
	CardsAdd(CARD_ANCIENT_UNITS_BRONZE_CLAD_HULL);
	CardsAdd(CARD_ANCIENT_UNITS_SPOKED_WHEELS);
	CardsAdd(CARD_ANCIENT_UNITS_SHIELD_WALL);
	CardsAdd(CARD_ANCIENT_UNITS_VALOUR);
	CardsAdd(CARD_ANCIENT_RESOURCES_INUITS);
	CardsAdd(CARD_ANCIENT_RESOURCES_BEDOUINS);
	CardsAdd(CARD_ANCIENT_RESOURCES_ATLATL);
	CardsAdd(CARD_ANCIENT_RESOURCES_FLINT_KNAPPING);
	CardsAdd(CARD_ANCIENT_RESOURCES_SACRIFICIAL_LAMBS);
	CardsAdd(CARD_ANCIENT_RESOURCES_SPEAR_FISHING);
	CardsAdd(CARD_ANCIENT_RESOURCES_DIVINE_CREATION);
	CardsAdd(CARD_ANCIENT_BUILDINGS_SHIPS_OF_THE_DESERT);
	CardsAdd(CARD_ANCIENT_BUILDINGS_FORCED_LEVY);
	CardsAdd(CARD_ANCIENT_POLITICAL_ORTHODOXY);
	CardsAdd(CARD_ANCIENT_POLITICAL_PROGRESSIVE);
	CardsAdd(CARD_ANCIENT_POLITICAL_AGGRESIVE);
	CardsAdd(CARD_ANCIENT_POLITICAL_EXCLUSIVE);
	CardsAdd(CARD_ANCIENT_POLITICAL_PROTECTIVE);
	CardsAdd(CARD_ANCIENT_BUILDINGS_DRUIDS);
	CardsAdd(CARD_ANCIENT_BUILDINGS_HARBORMASTER);
	CardsAdd(CARD_CLASSICAL_RESOURCE_LIMESTONE);
	CardsAdd(CARD_CLASSICAL_UNITS_INDUSTRIOUS);
	CardsAdd(CARD_CLASSICAL_UNITS_FORCED_MARCH);
	CardsAdd(CARD_CLASSICAL_UNITS_SIEGE_ENGINEERS);
	CardsAdd(CARD_CLASSICAL_UNITS_SUPPLIES);
	CardsAdd(CARD_CLASSICAL_UNITS_FLETCHING);
	CardsAdd(CARD_CLASSICAL_WONDERS_PHEIDIAS);
	CardsAdd(CARD_CLASSICAL_BUILDINGS_BEACONS_OF_HOPE);
	CardsAdd(CARD_CLASSICAL_BUILDINGS_GLADIATOR_GAMES);
	CardsAdd(CARD_CLASSICAL_BUILDINGS_CORINTHIAN_ORDER);
	CardsAdd(CARD_CLASSICAL_LEGENDARY_CHAMPION);
	CardsAdd(CARD_CLASSICAL_BUILDINGS_FLUTE_AND_LYRE);
	CardsAdd(CARD_MEDIEVAL_RESOURCE_GILLNETS);
	CardsAdd(CARD_MEDIEVAL_RESOURCE_PRECIOUS_METALS);
	CardsAdd(CARD_MEDIEVAL_RESOURCE_SILK_ROAD);
	CardsAdd(CARD_MEDIEVAL_RESOURCE_SPICE_TRADE);
	CardsAdd(CARD_MEDIEVAL_RESOURCE_EXTRAVAGANCE);
	CardsAdd(CARD_MEDIEVAL_RESOURCE_VITICULTURE);
	CardsAdd(CARD_MEDIEVAL_UNITS_CHAINMAIL_ARMOR);
	CardsAdd(CARD_MEDIEVAL_UNITS_LANTEEN_SAILS);
	CardsAdd(CARD_MEDIEVAL_UNITS_REPEATING_CROSSBOWS);
	CardsAdd(CARD_MEDIEVAL_UNITS_SERFS);
	CardsAdd(CARD_MEDIEVAL_UNITS_FEUDALISM);
	CardsAdd(CARD_MEDIEVAL_WONDERS_CHICHEN_ITZA);
	CardsAdd(CARD_MEDIEVAL_WONDERS_BOROBUDUR);
	CardsAdd(CARD_CLASSICAL_WONDERS_MAUSOLEUM_OF_HALICARNASSUS);
	CardsAdd(CARD_CLASSICAL_WONDERS_PARTHENON);
	CardsAdd(CARD_ANCIENT_WONDERS_STATUE_OF_ZEUS);
	CardsAdd(CARD_ANCIENT_WONDERS_HIPPODAMUS_OF_MILETUS);
	CardsAdd(CARD_ANCIENT_WONDERS_TEMPLE_OF_ARTEMIS);
	CardsAdd(CARD_CLASSICAL_BUILDINGS_CANNON_OF_TEN);
	CardsAdd(CARD_MEDIEVAL_BUILDINGS_CAMBRIDGE_UNIVERSITY);
	CardsAdd(CARD_MEDIEVAL_BUILDINGS_FEALTY);
...
void CvPlayer::setStrike(bool bNewValue)
{
	if(m_bStrike != bNewValue)
	{
		...
		if(m_bStrike)
		{
			if(GetID() == GC.getGame().getActivePlayer())
			{
				GC.messagePlayer(0, GetID(), false, GC.getEVENT_MESSAGE_TIME(), GetLocalizedText("TXT_KEY_MISC_UNITS_ON_STRIKE").GetCString(), "AS2D_STRIKE", MESSAGE_TYPE_MINOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
					...
					if(pLoopPlot->isVisible(getTeam()))
					{
						...
						if(pUnit != NULL)
						{
							...
							if(pNearestCity != NULL)
							{
								...
								GC.messagePlot(0, pLoopPlot->GetPlotIndex(), GetID(), true, GC.getEVENT_MESSAGE_TIME(), message);
