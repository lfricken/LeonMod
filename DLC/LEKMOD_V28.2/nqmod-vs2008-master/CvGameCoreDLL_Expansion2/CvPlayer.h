...
public:
	...
	// recursive call is used to make sure buildings don't have maintenance AND positive gold
	int GetTotalYieldForBuilding(const CvCity* pCity, const BuildingTypes eBuilding, const YieldTypes eYieldType, const bool isPercentMod = false, const bool inRecursiveCall = false) const;
...
public:
	...
	bool IsCapitalConnectedToCity(CvCity* pCity, RouteTypes eRestrictRouteType = NO_ROUTE, const bool bIgnoreHarbors = false);
	...
	T100 GetTourismCombatPenaltyT100(const PlayerTypes eOtherPlayerId) const;
	...
	void getNumAllowed(BuildingClassTypes eIndex, int* have, int* allowed) const;
