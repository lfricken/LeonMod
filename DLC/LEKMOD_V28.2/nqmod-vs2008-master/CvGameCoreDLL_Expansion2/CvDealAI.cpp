...
int CvDealAI::GetTradeItemValue(TradeableItems eItem, bool bFromMe, PlayerTypes eOtherPlayer, int iData1, int iData2, int iData3, bool bFlag1, int iDuration, bool bUseEvenValue)
{
	...
	if (eItem == TRADE_ITEM_GOLD)
		...
		iItemValue = TradingCard::GetEstimatedValue(bFromMe, (TradingCardTypes)iData2);
	else if (eItem == TRADE_ITEM_LUMP)
		iItemValue = GetResourceValue(/*ResourceType*/ (ResourceTypes)iData1, /*Quantity*/ iData2, 1, bFromMe, eOtherPlayer);
...
int CvDealAI::GetOpenBordersValue(bool bFromMe, PlayerTypes eOtherPlayer, bool bUseEvenValue)
{
	...
	if(bFromMe)
	{
		...
		if (eCultureStrategy != NO_AIGRANDSTRATEGY && GetPlayer()->GetGrandStrategyAI()->GetGuessOtherPlayerActiveGrandStrategy(eOtherPlayer) == eCultureStrategy)
		{
			...
			if (kOtherPlayer.GetCulture()->GetOurNetTourismT100() / 100 > 0 && kOtherPlayer.GetCulture()->GetInfluenceOn(GetPlayer()->GetID()) < INFLUENCE_LEVEL_INFLUENTIAL)
			{
				iItemValue *= 500;
				iItemValue /= 100;
			}
	...
	else
	{
		...
		if (eCultureStrategy != NO_AIGRANDSTRATEGY && GetPlayer()->GetGrandStrategyAI()->GetActiveGrandStrategy() == eCultureStrategy && pPlayerCulture->GetOurNetTourism() > 0)
		{
...
#else
		if (eCultureStrategy != NO_AIGRANDSTRATEGY && GetPlayer()->GetGrandStrategyAI()->GetActiveGrandStrategy() == eCultureStrategy && GetPlayer()->GetCulture()->GetOurNetTourismT100() / 100 > 0 )
		{
			// The civ we need influence on the most should ALWAYS be included
