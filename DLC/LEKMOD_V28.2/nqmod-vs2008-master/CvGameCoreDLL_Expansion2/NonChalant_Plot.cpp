...
bool checkAndUpdateCache(std::vector<int> cacheData, int num, int* data)
{
	if (num != cacheData.size())
		return false;
	bool wasCacheGood = true;
	for (int i = 0; i < num; ++i)
	{
...
bool CvPlot::checkPolicyCache(int num, int* data) const
{
	return checkAndUpdateCache(m_cachedExtraYields_PoliciesData, num, data);
}
bool CvPlot::checkTechCache(int num, int* data) const
{
	return checkAndUpdateCache(m_cachedExtraYields_TechData, num, data);
}
bool CvPlot::checkBeliefCache(int num, int* data) const
{
	return checkAndUpdateCache(m_cachedExtraYields_BeliefsData, num, data);
}

int getExtraYield_BeliefsChanged
(
	const CvPlot& plot,
	const CvCity* shity,
...
)
{
	...
	const ReligionTypes majorityReligion = shity->GetCityReligions()->GetReligiousMajority(); // the majority religion in this city
	...
	const int numFollowersGlobal = GC.getGame().GetGameReligions()->GetNumFollowers(majorityReligion); // number of people following the majority religion in this city globaly
	...
	const bool isHolyCity = shity->GetCityReligions()->IsHolyCityForReligion(majorityReligion); // true if this is the holy city of the majority religion in this city
	const int numFollowersLocal = shity->GetCityReligions()->GetNumFollowers(majorityReligion); // number of people following the majority religion in this city

	int beliefData[5] = { 16, 2, 77, 40, 12071 };
	const bool useCache = plot.checkTechCache(4, beliefData);
	if (!useCache)
	{
		int yieldChange = 0;
		const CvCity_InterfaceBelief* city = shity;
		...
		m_cachedExtraYields_Beliefs[eYieldType] = yieldChange;
	}

	return m_cachedExtraYields_Beliefs[eYieldType];
}
...
(
	const CvPlot& plot,
	const CvPlayer_InterfacePolicy* player,
	// type of yield we are considering
	const YieldTypes eYieldType,
	// type of improvement
	...
	// owning player
	const PlayerTypes tileOwner,
	const bool isGreatTile
)
{


	const bool useCache = m_cachedExtraYields_Policies[eYieldType] != InvalidCacheVal;
	if (!useCache)
	{
		int yieldChange = 0;
		...
		if (tileOwner != NO_PLAYER)
		{
			...
			const CvPlayer_InterfacePolicy* player = &GET_PLAYER(tileOwner);
...
(
	const CvPlot& plot,
	const CvPlayer_InterfaceTech* player,
	// type of yield we are considering
	const YieldTypes eYieldType,
	// type of improvement
	const ImprovementTypes,
...
{
	const bool useCache = plot.m_cachedExtraYields_Tech[eYieldType] != InvalidCacheVal;
	if (!useCache)
	{
		int yieldChange = 0;
		const CvPlot& plot = *this;
		...
		plot.m_cachedExtraYields_Tech[eYieldType] = yieldChange;
	}
	return plot.m_cachedExtraYields_Tech[eYieldType];
}
...
)
{
	...
	{
		const CvPlayer* player = &GET_PLAYER(tileOwner);
		yieldChange += getExtraYield_TechChanged(plot, player, eYieldType, eImprovement, eRoute, tileOwner);
		yieldChange += getExtraYield_PoliciesChanged(plot, player, eYieldType, eImprovement, eRoute, tileOwner, isGreatTile);
	...
	if (pWorkingCity != NULL)
	{
		...
		yieldChange += getExtraYield_BeliefsChanged(*this, pWorkingCity, eYieldType, eImprovement, eRoute, tileOwner);
	}
