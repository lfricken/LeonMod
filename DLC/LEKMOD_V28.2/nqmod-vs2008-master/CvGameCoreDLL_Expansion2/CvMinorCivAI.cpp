...
string religionToReligionName(const ReligionTypes eReligion)
{
	...
	if (eReligion != NO_RELIGION)
	{
		...
		if (pkReligion != NULL)
		{
			const string religionNameKey = pkReligion->GetName();
			const CvReligionEntry* pInfo = GC.getReligionInfo(eReligion);

			size_t numBytes;
			stringstream s;
			s << pInfo->GetIconString();
			s << " ";
			s << Localization::Lookup(religionNameKey.c_str()).GetBaseString(numBytes);

			strReligionName = s.str();
		}
	}
	else
		strReligionName = "Atheism";
...
{
	const int winner = GetContestValueForPlayer(ePlayer);
	const int you = GetContestValueForPlayer(ePlayer);
	stringstream s;
	if (winner <= 0) // nobody winning
		s << "Nobody is impressing us currently.";
	else if (winner > you) // losing
		s << "You're [COLOR_NEGATIVE_TEXT]losing with " << you << "[ENDCOLOR] while the leader has " << winner << ".";
	else // winning
		s << "You're [COLOR_POSITIVE_TEXT]winning with " << you << "[ENDCOLOR].";
	return s.str();
}

string reward(const string colorBegin, const int amount, const string unit)
{
	stringstream s;
	string colorEnd = "";
	if (colorBegin != "")
	...
	s << colorBegin << sign << abs(amount) << colorEnd << unit;
	return s.str();
}
string compileRewards(QuestReward rewards)
{
	vector<string> ss;
	const string colorBegin = "";
	if (rewards.friendship > 0)
	{
		ss.push_back(reward(colorBegin, rewards.friendship, " {FRIENDSHIP}"));
	}
...
string CvMinorCivQuest::GetStatusMessage(const CvPlayer* pMinor, const CvPlayer* pMajor, MessageType messageType, string value3) const
{
	...
	{
		s << civName + " has a new quest for you. (also, bug report this message)";
	}

	return GetLocalizedText(s.str().c_str());
}
...
int CvMinorCivQuest::GetEndTurn() const
{
	...
	int iLength = 0;
	if (m_eType == QUEST_UNREST) // contests
	{
...
int CvMinorCivQuest::GetTurnsRemaining(int iCurrentTurn) const
{
	...
	if (iStartTurn == NO_TURN || iEndTurn == NO_TURN)
		return NO_TURN;
...
bool CvMinorCivQuest::IsComplete()
{
	...
	if(m_eType == QUEST_BUILD_WORLD_WONDER)
	{
		...
		const bool haveMore = currentCount > oldCount;
		return haveMore;
	}
	// handled in gift code
	...
	else if(m_eType == QUEST_SPREAD_RELIGION)
	{
		...
		if(eReligion != NO_RELIGION && pMinor->getCapitalCity())
		{
...
void CvMinorCivQuest::DoStartQuest(int iStartTurn)
{
	...
	if (m_eType == QUEST_BUILD_WORLD_WONDER)
	{
		const int currentCount = pMajor->GetNumWonders();
	...
	{
		const int currentCount = pMajor->getNumNationalWonders();
		m_iData1 = currentCount;
	}
	else if (m_eType == QUEST_GIFT_GOLD)
	{
	...
	strMessage = GetStatusMessage(pMinor, pMajor, START_DESC, "").c_str();
	strSummary = GetStatusMessage(pMinor, pMajor, START_CAPTION, "").c_str();
...
bool CvMinorCivQuest::DoFinishQuest(const T100 bonusFactorT100)
{
	...
	// apply rewards
	pMinor->GetMinorCivAI()->ChangeFriendshipWithMajorTimes100(m_eAssignedPlayer, friendshipRewardT100, /*bFromQuest*/ true);
	...
	CvString sMessage = GetStatusMessage(pMinor, pMajor, WIN_DESC, "").c_str();
	CvString sSummary = GetStatusMessage(pMinor, pMajor, WIN_CAPTION, "").c_str();
...
bool CvMinorCivQuest::DoCancelQuest()
{
	...
	{
		CvString sMessage = GetStatusMessage(pMinor, pMajor, LOSE_DESC, "").c_str();
		CvString sSummary = GetStatusMessage(pMinor, pMajor, LOSE_CAPTION, "").c_str();
...
void CvMinorCivAI::Reset()
{
	...
	for(iI = 0; iI < MAX_MAJOR_CIVS; iI++)
	{
		...
		m_aiFriendshipWithMajorTimes100[iI] = 0; // class init
...
{
	// this is called when PlayerTypes is -1, why? does it cause inefficiency or unused memory?
...
void CvMinorCivAI::DoChangeAliveStatus(bool bAlive)
{
	if (!bAlive)
	{
		...
		int changesT100[MAX_MAJOR_CIVS] = { 0 };
		for (int i = 0; i < MAX_MAJOR_CIVS; ++i)
		{
			...
			changesT100[i] = -GetBaseFriendshipWithMajorTimes100(e);
		}
	...
	{
		// default friend update will run and apply benefits and notifications
	}
}
...
void CvMinorCivAI::DoThreateningBarbKilled(PlayerTypes eKillingPlayer, int iX, int iY, const bool isCamp)
{
	...
	ChangeFriendshipWithMajorTimes100(eKillingPlayer, amountFriendshipT100);
...
void CvMinorCivAI::DoTestStartPersonalQuest(PlayerTypes ePlayer)
{
	...
	// too early?
	if (!ENABLE_QUESTS_AT_START)
	{
		const int iFirstTurnForPersonalQuests = GetFirstPossibleTurnForPersonalQuests();
	...
	for(int iQuestLoop = 0; iQuestLoop < NUM_MINOR_CIV_QUEST_TYPES; iQuestLoop++)
	{
		const MinorCivQuestTypes eQuest = (MinorCivQuestTypes) iQuestLoop;
		if(IsValidQuestForPlayer(ePlayer, eQuest) && IsPersonalQuest(eQuest))
		{
			const int iCount = GetPersonalityQuestBias(eQuest);
			for(int iCountLoop = 0; iCountLoop < iCount; iCountLoop++)
			{
				veValidQuests.push_back(eQuest);
			}
	...
	for (itr_quest = m_QuestsGiven[ePlayer].begin(); itr_quest != m_QuestsGiven[ePlayer].end(); itr_quest++)
	{
		if (bCheckAllQuests || itr_quest->GetType() == eSpecifyQuestType)
		{
			if (itr_quest->IsComplete())
			{
				...
				const bool bCompleted = itr_quest->IsComplete();
				if (bCompleted) itr_quest->DoFinishQuest();
...
void CvMinorCivAI::DoObsoleteQuestsForPlayer(PlayerTypes ePlayer, MinorCivQuestTypes eSpecifyQuestType)
{
	...
	{
		const int minQuestFriendship = GC.getMINOR_FRIENDSHIP_ANCHOR_MOD_WARY_OF();
...
bool CvMinorCivAI::IsValidQuestForPlayer(const PlayerTypes ePlayer, const MinorCivQuestTypes eQuest)
{
	...
	const int currentFriendship = GetBaseFriendshipWithMajorTimes100(ePlayer);
	const int minQuestFriendship = GC.getMINOR_FRIENDSHIP_ANCHOR_MOD_WARY_OF();
	if (currentFriendship < minQuestFriendship)
		return false;
	...
	if (eQuest == QUEST_SPREAD_RELIGION)
	{
	...
	// TODO check to see if they already have all national wonders OR all world wonders are built
...
bool CvMinorCivAI::IsValidQuestCopyForPlayer(PlayerTypes ePlayer, CvMinorCivQuest* pQuest)
{
	...
	return IsValidQuestForPlayer(ePlayer, pQuest->GetType());
}
...
string CvMinorCivAI::GetQuestDescription(const PlayerTypes eMajor, const MinorCivQuestTypes eType, const MessageType messageType) const
{
	for (uint iQuestLoop = 0; iQuestLoop < m_QuestsGiven[eMajor].size(); iQuestLoop++)
	{
		const CvMinorCivQuest quest = m_QuestsGiven[eMajor][iQuestLoop];
		if (quest.GetType() == eType)
		{
			return quest.GetStatusMessage(m_pPlayer, &GET_PLAYER(eMajor), messageType, "");
		}
	}
...
void CvMinorCivAI::DoFriendship()
{
	...
	ChangeAndUpdateFriendship(m_aiFriendshipDeltaWithMajorTimes100);
	for (int i = 0; i < MAX_MAJOR_CIVS; ++i)
	{
...
void CvMinorCivAI::SetAndUpdateFriendshipSelective(const int setsNegativeOneT100[MAX_MAJOR_CIVS])
{
	...
	for (int i = 0; i < MAX_MAJOR_CIVS; ++i)
	{
		...
		{
			sets[i] = GetBaseFriendshipWithMajorTimes100(e);
		}
		else
		{
			sets[i] = setsNegativeOneT100[i];
		}
	}
...
void CvMinorCivAI::ChangeAndUpdateFriendship(const int changesT100[MAX_MAJOR_CIVS])
{
	...
	for (int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
	{
		...
		if (GET_PLAYER(ePlayer).isAlive())
		{
			const int iChangeThisApplicationT100 = changesT100[ePlayer];

			const T100 iOldFriendshipT100 = GetBaseFriendshipWithMajorTimes100(ePlayer);
			T100 iNewFriendshipT100 = iOldFriendshipT100 + iChangeThisApplicationT100;

			// stay in range
			if (iNewFriendshipT100 < minAnchorT100)
				iNewFriendshipT100 = minAnchorT100;
			else if (iNewFriendshipT100 > maxAnchorT100)
				iNewFriendshipT100 = maxAnchorT100;

			// friendship changed
			if (iNewFriendshipT100 != iOldFriendshipT100)
			{
				SetFriendshipWithMajorTimes100(ePlayer, iNewFriendshipT100); // this is the official place to change friendship
			}

			// find the new ally
			if (iNewFriendshipT100 >= GetAlliesThreshold() * 100) // if qualifies for ally
			{
				// if higher, they win
				if (iNewFriendshipT100 > highestAllyFriendshipT100)
				{
					highestAllyPlayer = ePlayer;
					highestAllyFriendshipT100 = iNewFriendshipT100;
				}
				// if tied, current ally wins
				else if (iNewFriendshipT100 == highestAllyFriendshipT100 && ePlayer == previousAlly)
				{
					highestAllyPlayer = ePlayer;
					highestAllyFriendshipT100 = iNewFriendshipT100;
				}
	...
	// apply effects of friendship number changes
	for (int i = 0; i < MAX_MAJOR_CIVS; i++)
	{
		const PlayerTypes ePlayer = (PlayerTypes)i;
		const int newTotalFriendshipT100 = GetBaseFriendshipWithMajorTimes100(ePlayer);
		// even if Friendship amount doesn't change, but ally state could have (ex. current ally decays below our level)
		UpdateOnAnyFriendshipChangeEffects(previousAlly, highestAllyPlayer, ePlayer, oldFriendshipT100[i], newTotalFriendshipT100);
	}

	SetAlly(highestAllyPlayer);	// We KNOW no one else can be higher, so set the Ally to NO_PLAYER
}


int newCappedChange(const int currentT100, const int newT100, const int anchorT100)
{
	if (newT100 > currentT100) // increase
	{
...
int CvMinorCivAI::GetFriendshipChangePerTurnTimes100(const PlayerTypes ePlayer)
{
	...
	int newPredictedT100 = currentValueT100;

	// move toward anchor
	{
		const int baseAnchorT100 = GetFriendshipAnchorWithMajor(ePlayer) * 100;
		const int defaultChangeT100 = -GC.getMINOR_FRIENDSHIP_DROP_PER_TURN();
		int changeT100 = 0;
		if (currentValueT100 < baseAnchorT100)
			changeT100 += defaultChangeT100;
		else if (currentValueT100 > baseAnchorT100)
			changeT100 -= defaultChangeT100;
		...
		newPredictedT100 = newCappedChange(newPredictedT100, newPredictedT100 + changeT100, baseAnchorT100);
	}

	// handle military
	{
		const int militaryAnchorLowT100 = 100 * 100;
		const int militaryAnchorHighT100 = maxAnchorT100;
	...
	// prevent moving past min or max
	if (newPredictedT100 < currentValueT100) // drop
		newPredictedT100 = newCappedChange(currentValueT100, newPredictedT100, minAnchorT100);
...
void CvMinorCivAI::SetFriendshipWithMajorTimes100(PlayerTypes ePlayer, int iNum, bool)
{
	...
	//{
	//	UdpdateAnyFriendshipChangeEffects(ePlayer, iOldEffectiveFriendship/100, iNewEffectiveFriendship/100, bFromQuest);
	//}
...
void CvMinorCivAI::ChangeFriendshipWithMajorTimes100(PlayerTypes ePlayer, int iChange, bool bFromQuest)
{
	...
	if (iChange != 0)
	{
		...
		//SeetFriendshipWithMajorTimes100(ePlayer, GetBaseFriendshipWithMajorTimes100(ePlayer) + iChange, bFromQuest);
	}
}
void CvMinorCivAI::ChangeFriendshipWithMajorTimes100Instant(PlayerTypes ePlayer, int iChangeT100, bool bFromQuest)
{
	...
	{
		// If this friendship was earned from a Quest, then we might apply a modifier to it
		if (bFromQuest && iChangeT100 > 0)
		{
			if (GET_PLAYER(ePlayer).getMinorQuestFriendshipMod() != 0)
			{
				iChangeT100 *= (100 + GET_PLAYER(ePlayer).getMinorQuestFriendshipMod());
				iChangeT100 /= 100;
			}
		}
		int changesT100[MAX_CIV_PLAYERS] = { 0 }; // all elements 0
		changesT100[ePlayer] = iChangeT100;
		ChangeAndUpdateFriendship(changesT100);
	}
}
...
void CvMinorCivAI::OnChangeFriendStatus(const bool isAtLeastFriends, const PlayerTypes ePlayer, const int iOldFriendshipT100, const int iNewFriendshipT100, const bool suppressNotifs)
{
	DoSetBonus(ePlayer, isAtLeastFriends, true, false, suppressNotifs);
	...
	if (pkScriptSystem)
	{
		...
		args->Push(m_pPlayer->GetID());
		args->Push(ePlayer);
		args->Push(isAtLeastFriends);
		args->Push(iOldFriendshipT100);
		args->Push(iNewFriendshipT100);

		bool bResult;
		LuaSupport::CallHook(pkScriptSystem, "MinorFriendsChanged", args.get(), bResult);
	}
}
void CvMinorCivAI::OnChangeAllyStatus(const bool isAtLeastAllies, const PlayerTypes ePlayer, const int iOldFriendshipT100, const int iNewFriendshipT100, const bool suppressNotifs)
{
	DoSetBonus(ePlayer, isAtLeastAllies, false, true, suppressNotifs);
...
void CvMinorCivAI::UpdateOnAnyFriendshipChangeEffects(const PlayerTypes ePreviosAlly, const PlayerTypes newAlly, PlayerTypes ePlayer, int iOldFriendshipT100, int iNewFriendshipT100, bool bFromQuest, bool bIgnoreMinorDeath)
{
	// avoid giving out bonuses if we're dead
	...
	{ // friend status updates
		const bool wasFriends = IsFriendshipAboveFriendsThreshold(iOldFriendshipT100 / 100);
		const bool isNowFriends = IsFriendshipAboveFriendsThreshold(iNewFriendshipT100 / 100);
		...
		if (wasFriends != isNowFriends)
		{
			OnChangeFriendStatus(isNowFriends, ePlayer, iOldFriendshipT100, iNewFriendshipT100, /*bSuppressNotifications*/ bFromQuest);
		}
	}

	{ // ally status updates
		const bool wasAlly = ePlayer == ePreviosAlly;
		const bool isNowAlly = ePlayer == newAlly;

		// friendship state changed
		if (wasAlly != isNowAlly)
		{
			OnChangeAllyStatus(isNowAlly, ePlayer, iOldFriendshipT100, iNewFriendshipT100, /*bSuppressNotifications*/ bFromQuest);
		}
	}
...
void CvMinorCivAI::DoLiberationByMajor(PlayerTypes eLiberator, TeamTypes eConquerorTeam)
{
	...
	const int liberateFriendshipT100 = 100 * GC.getMINOR_LIBERATION_FRIENDSHIP();

	int setsT100[MAX_CIV_PLAYERS] = { 0 };
	for(int iI = 0; iI < MAX_MAJOR_CIVS; iI++)
	{
		...
		{
			setsT100[iI] = 0; // didn't liberate us, you get 0 influence
			...
			if(GET_PLAYER(ePlayer).isAlive())
			{
				...
				{
					// your team conquered us
					if(GET_PLAYER(ePlayer).getTeam() == eConquerorTeam)
					{
						setsT100[iI] = 100 * GC.getMINOR_FRIENDSHIP_AT_WAR(); // conquered us? we hate you
					}
		...
		{
			// ally with liberator
			setsT100[iI] = liberateFriendshipT100;
		}
	}
	SetAndUpdateFriendship(setsT100);
	...
	strMessage << GetPlayer()->getNameKey();
	s << strMessage.toUTF8();
	s << "[NEWLINE][ICON_BULLET]+" << (liberateFriendshipT100 / 100) << " {FRIENDSHIP}";
...
CvString CvMinorCivAI::GetMajorBullyUnitDetails(PlayerTypes ePlayer)
{
	...
	UnitTypes eUnitType = (UnitTypes) GC.getInfoTypeForString("UNIT_WORKER"); // XML/function
...
bool CvMinorCivAI::IsRecentlyBulliedByMajor(PlayerTypes ePlayer) const
{
	...
	const int iRecentlyBulliedTurnInterval = 20; // constant/XML
...
void CvMinorCivAI::DoUnitGiftFromMajor(PlayerTypes eFromPlayer, CvUnit* pGiftUnit, bool bDistanceGift)
{
	...
	const bool isMilitary = pGiftUnit->GetBaseCombatStrengthConsideringDamage();

	// check gift completion
	QuestListForPlayer::iterator itr_quest;
	for (itr_quest = m_QuestsGiven[eFromPlayer].begin(); itr_quest != m_QuestsGiven[eFromPlayer].end(); itr_quest++)
	{
		...
		(
			...
			(thisType == QUEST_GIFT_MILITARY && isMilitary) ||
			(thisType == QUEST_GIFT_WORKER && isWorker)
		);

		// great person
		if (thisType == QUEST_GIFT_GREAT_PERSON && isGreat)
		{
		...
		if (thisSatisfied)
		{
			itr_quest->DoFinishQuest(rewardFactor);
			break; // only satisfy one quest per gift!
		}
	}
...
int CvMinorCivAI::GetFriendshipFromUnitGift(PlayerTypes eFromPlayer, bool bGreatPerson, bool /*bDistanceGift*/, const bool isCombat)
{
	CvAssertMsg(eFromPlayer >= 0, "eFromPlayer is expected to be non-negative (invalid Index)");
	CvAssertMsg(eFromPlayer < MAX_MAJOR_CIVS, "eFromPlayer is expected to be within maximum bounds (invalid Index)");
	...
	else
	{
		...
		// War state
		if (isCombat && IsProxyWarActiveForMajor(eFromPlayer))
		{
			iInfluence *= 2; // XML
		}
...
void CvMinorCivAI::DoGoldGiftFromMajor(const PlayerTypes ePlayer, const int iGold)
{
	...
	if (iGold > 0)
	{
		...
		if (canAfford)
		{
			int iFriendshipChangeT100 = GetFriendshipFromGoldGift(ePlayer, iGold) * 100;
			if (iFriendshipChangeT100 > 0)
			...
			// cap friendship from gold
			const int maxGoldFriendshipT100 = GC.getMINOR_CIV_MAX_GOLD_FRIENDSHIP() * 100;
			const int currentT100 = GetBaseFriendshipWithMajorTimes100(ePlayer);
			if (currentT100 + iFriendshipChangeT100 > maxGoldFriendshipT100)
			{
				iFriendshipChangeT100 = maxGoldFriendshipT100 - currentT100;
			}


			ChangeFriendshipWithMajorTimes100(ePlayer, iFriendshipChangeT100);
...
{
	int iWarFriendshipT100 = /*-100*/ GC.getMINOR_FRIENDSHIP_AT_WAR() * 100;

	int changesT100[MAX_CIV_PLAYERS] = { 0 };
	...
	for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
	{
		...
		{
			// Friendship change
			changesT100[iPlayerLoop] = iWarFriendshipT100 - GetBaseFriendshipWithMajorTimes100(ePlayer) - m_aiFriendshipDeltaWithMajorTimes100[iPlayerLoop];
...
void CvMinorCivAI::DoNowPeaceWithTeam(TeamTypes)
{
	...
	//	{
	//		// Friendship change
	//		const int iNewFriendship = GetBaseFriendshipWithMajor(ePlayer);
	//		//UpdateAnyFriendshipChangeEffects(ePlayer, iWarFriendship, iNewFriendship);
	//	}
	//}

	int changesT100[MAX_CIV_PLAYERS] = { 0 };
	ChangeAndUpdateFriendship(changesT100); // force an update
...
void CvMinorCivAI::DoTeamDeclaredWarOnMe(TeamTypes eEnemyTeam)
{
	...
	{
		int iMinimumFriendshipMod = 20; // xml
...
FDataStream& operator<<(FDataStream& saveTo, const QuestReward& readFrom)
{
	...
	saveTo << readFrom.goldenPoints;

	saveTo << readFrom.hammers;
	saveTo << readFrom.insight;
	saveTo << readFrom.militaryUnits;

	saveTo << readFrom.support;

	return saveTo;
}
FDataStream& operator>>(FDataStream& loadFrom, QuestReward& writeTo)
{
	...
	loadFrom >> writeTo.goldenPoints;

	loadFrom >> writeTo.hammers;
	loadFrom >> writeTo.insight;
	loadFrom >> writeTo.militaryUnits;

	loadFrom >> writeTo.support;
